 ...
 * @param argv
 */
pid_t handle_parameter(int argc, char *argv[]){
    pid_t pid = 0;
    int index = 0;
    char *pkg_name = NULL;
    char *lib_path = NULL;
...
int init_inject(int argc, char *argv[]){
    ...
    // 参数处理
    pid_t traget_pid = handle_parameter(argc, argv);
    if(!init_system_lib_Info(traget_pid)){
        printf("[-][InitLib] init_system_lib_Info Failed \n");
    }
